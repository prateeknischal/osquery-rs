...
impl ExtensionManagerClient {
    ...
    pub fn new_with_path(path: &str, uuid: Option<i64>) -> Self {
        ...
        if cfg!(target_os = "windows") {
            ...
            let socket = unix_socket::UnixStream::connect(&path).expect(&format!(
                "Unable to connect to {} with unix_socket::UnixStream",
                path
            ));
...
impl ExtensionSyncHandler for ExtensionClientHandler {
    ...
    fn handle_call(
        ...
        if (*reg).contains_key(&registry) == false {
            ...
            let app_err =
                thrift::ApplicationError::new(kind, format!("Unknown registry: {}", registry));
        ...
        if sub_registry.contains_key(&item) == false {
            ...
            let app_err = thrift::ApplicationError::new(
                kind,
                format!(
                    "Unknown plugin ({}) requested from registry ({})",
                    item, registry
                ),
            );
            return Err(thrift::Error::Application(app_err));
...
impl ExtensionManagerServer {
    ...
    pub fn run(self) {
        ...
        std::thread::spawn(move || {
            ...
            // Start a client that pings the osquery extension manager server. If the osquery
            // servers goes away, the extension manager will signal itself to stop using the done
            // signal handler.
            loop {
                match client.ping() {
                    Ok(_) => {}
...
mod tests {
    ...
    fn test_table() {
        ...
        let mut extension_server = ::ExtensionManagerServer::new_with_path(
            "test_thing123",
            "/Users/zbrown/.osquery/shell.em",
        );
